        ...
        {activeView === "overview" && (
          <>
            <header className="mb-8">
              ...
              <p className="text-slate-400">Real-time AWS processing logs for your company.</p>
            </header>

            {/* MOCK AWS LOGS AREA */}
            <div className="bg-black text-green-400 p-6 rounded-xl font-mono text-sm overflow-hidden shadow-2xl border border-white/5">
        ...
        {activeView === "post-job" && (
          <div className="max-w-2xl mx-auto bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-xl">
            <header className="mb-8">
              ...
              <p className="text-slate-400">Target the best talent with precise requirements.</p>
            </header>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                ...
                <input
                  required
                  className="w-full p-3 bg-slate-800 border border-slate-700 text-white rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none"
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <input
                    required
                    className="w-full p-3 bg-slate-800 border border-slate-700 text-white rounded-xl"
                ...
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-slate-300">Salary (USD)</label>
                  <input
                    type="number"
                    className="w-full p-3 bg-slate-800 border border-slate-700 text-white rounded-xl"
              ...
              <div className="pt-4 flex gap-4">
                <button type="submit" className="flex-1 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-colors flex items-center justify-center gap-2 shadow-lg shadow-blue-500/20">
                  <Save size={20} />
                ...
                <button
                  ...
                  className="px-6 py-3 border border-slate-700 hover:bg-slate-800 text-slate-300 font-bold rounded-xl transition-colors flex items-center gap-2"
                >
                  <Download size={20} />
                  Download PDF
                </button>
...
function NavItem({ icon, label, active = false, onClick }) {
  return (
    <div
      onClick={onClick}
      className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all ${active ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
    >
      {icon}
